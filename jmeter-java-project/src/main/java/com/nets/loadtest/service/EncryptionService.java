...
/**
 * Service class for handling AES key generation and RSA encryption operations. Provides methods to
 * generate AES keys, load RSA public keys, and encrypt AES keys with RSA.
 */
public class EncryptionService {
  ...
  /**
   * Encrypts an AES key using RSA public key encryption with OAEP padding. Encrypts the raw AES key
   * bytes directly (standard approach).
   *
  ...
  /**
   * Main method that orchestrates the encryption process: 1. Loads RSA public key from PEM 2.
   * Generates AES symmetric key 3. Stores AES key for later use 4. Encrypts AES key with RSA public
   * key
   *
   * @param publicKeyPem Base64-encoded RSA public key in PEM format
   * @return Base64-encoded encrypted AES key, or null if publicKeyPem is empty/null
  ...
  /**
   * Encrypts plaintext using AES-GCM with the provided key and nonce. Returns Base64-encoded result
   * containing nonce + ciphertext + auth tag.
   *
   ...
   * @throws Exception if encryption fails
   */
  public String encryptPasswordWithAes(String aesKeyBase64, String plaintext, String nonceBase64)
      throws Exception {
  ...
  /**
   * Encrypts plaintext to Base64 using AES-GCM. Combines nonce + ciphertext + auth tag and returns
   * as Base64.
   *

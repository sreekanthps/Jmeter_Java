...
public class CardEncryptionService {
  ...
  public static EncryptionResult encrypt(String publicKeyBase64, String payload) throws Exception {
    ...
    return new EncryptionResult(Base64.getEncoder().encodeToString(encryptedAesKey),
        Base64.getEncoder().encodeToString(encryptedBody), ivString);
  }

  public static EncryptionResult encryptCard(String publicKeyBase64, byte[] bodyBytes)
      throws Exception {
    // 1. Generate AES Session Key
    KeyGenerator keyGen = KeyGenerator.getInstance("AES");
    keyGen.init(AES_KEY_SIZE);
